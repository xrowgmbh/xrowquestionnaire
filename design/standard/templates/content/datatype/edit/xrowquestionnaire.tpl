...
<div class="context-information">
    ...
    <div class="break"></div>
           <label>
               <input type="checkbox" name="{$attribute_base}_xrowquestionnaire[{$attribute.id}][settings][user_loggedin]" {if is_set($attribute.content.settings.user_loggedin)}checked="true"{/if} value="1" />
         ...
         <label title="{"Beim aktiveren dieser option müssen die Teilnehmer einloggen."|i18n( 'xrowquestionnaire/datatype/edit' )}">
              <input type="checkbox" name="{$attribute_base}_xrowquestionnaire[{$attribute.id}][settings][play_once]" {if is_set($attribute.content.settings.play_once)}checked="true"{/if} value="1" />
        ...
        <i>{"Beim Aktiveren dieser Option müssen sich die Teilnehmer einloggen."|i18n( 'xrowquestionnaire/datatype/edit' )}</i>
         ...
         <label>
            <input type="checkbox" name="{$attribute_base}_xrowquestionnaire[{$attribute.id}][settings][captcha]" {if is_set($attribute.content.settings.captcha)}checked="true"{/if} value="1" /> 
            {'CAPTCHA aktivieren'|i18n( 'xrowquestionnaire/datatype/edit' )}
        </label>
    
        <label>
            <input type="checkbox" name="{$attribute_base}_xrowquestionnaire[{$attribute.id}][settings][lottery]" {if is_set($attribute.content.settings.lottery)}checked="true"{/if} value="1" />
