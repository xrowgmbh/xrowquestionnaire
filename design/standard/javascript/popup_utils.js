...
var eZOEPopupUtils = {
    ...
    selectByEmbedId: function( id, node_id, name, useNode )
    {
        ...
        if ( id !== undefined )
        {
            ...
            var id = jQuery("iframe.uploadFrame_xrowquestionnaire", topDocument).attr('id').substring( 12 )
                idArray = id.split('_');
            $("input#"+ id +"_relation", topDocument).val(node_id);
            $('#mce_' + idArray[3], topDocument).remove();
            $('#mceModalBlocker', topDocument).remove();
        }
    },
...
if(jQuery('input.input_noborder'))
{
    jQuery('input.input_noborder').bind('click', function() {
        alert('User clicked on "foo."');
        //test( n.contentobject_id, n.node_id, n.name );
    });
}